...
1.  **Backend (`backend/api/views.py`):** Create `GET` and `POST` endpoints at `/api/settings/` to retrieve and update the `SystemSettings` singleton model (fields: `active_ai_provider`, `api_key`, `monthly_budget_target`). 
2.  **Security Note:** When returning the GET request, mask the `api_key` (e.g., `sk-...1234`) so it isn't fully exposed to the frontend after saving.
3.  **Frontend (`frontend/src/features/settings/SettingsForm.jsx`):** Build a simple form to select the AI Provider from a dropdown and input the API Key and Monthly Budget.
...
1.  Create a function `generate_financial_advice(aggregated_data, provider, api_key)`.
...
3.  **The Integration:** Write a basic HTTP wrapper (using `requests` or `httpx`) to send the prompt to the selected provider's API endpoint (OpenAI chat/completions style).
